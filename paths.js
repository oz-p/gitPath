...
paths = {	
	...
	// params fromRepId - string (optional): a repository id, if not empty value then the given repository will be ignored 
	findUsersPath: function(userId1,userId2,fromRepId) {	
		...
		for (var r in u1Repos) {
			...
			if (fromRepId && fromRepId==repoId) continue;
			...
			if (repoUserIds)
			{
				...
				{	
					...
					else if (ruId!=userId1)
					{
						...
						var uHopsResult = this.findUsersPath(ruId,userId2,repoId);
				...
				if (nUsersHops<result.hops || result.hops==0) 
				{
